...
class AI(QtCore.QThread):
    ...
    finishSignal = QtCore.pyqtSignal(cb.Point)
    ...
    def run(self):
        ...
        print(f"AI search score:{score}, x={x}, y={y}")
        # AI执白
        self.finishSignal.emit(cb.Point(x, y, config.State.WHITE))
...
class GoBang(QWidget):
    ...
    def initUI(self):
        ...
        self.chessboard = cb.ChessBoard()  # 棋盘类
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton: # 按下鼠标左键
            if not self.ai_down: # AI 还未下棋
                print("Not your turn!")
                return
            ui_point = cb.Point(e.x(), e.y(), config.State.BLACK)  # 鼠标坐标
            point = self.coordinate_transform_pixel2map(ui_point)  # 对应棋盘坐标
            if not point:  # 棋子落在棋盘外
                print("Out of chessboard!")
                return
            if self.chessboard.get_state(point) != config.State.EMPTY:  # 棋子没有落在空白处
    ...
    def AI_move(self, point: cb.Point):
        """轮到AI下棋"""
    ...
    def draw(self, point: cb.Point):
        """绘制落子位置的棋子图"""
        self.ui_point = self.coordinate_transform_map2pixel(point)
        
        label = QLabel(self)
        if point.state == config.State.BLACK:
        ...
        winner = self.chessboard.check_win(point)  # 判断输赢
        if winner != False:
    ...
    def coordinate_transform_map2pixel(self, point: cb.Point):
        ...
        return cb.Point(int(ui_x), int(ui_y), point.state)

    def coordinate_transform_pixel2map(self, ui_point:cb.Point):
        ...
        if x < 0 or x >= config.Config.SIZE or y < 0 or y >= config.Config.SIZE:
            ...
            return cb.Point(x, y, ui_point.state)

    def gameover(self, winner):
        if winner == config.State.BLACK:
            self.sound_win.play()
